...
$(document).ready(function () {
    ...
    function (event) {
      ...
      if (!window.isScroll && !(window.isMobile && window.isSimplifiedMobile)) {
        ...
        if (currentSectionHeight > window.windowHeight) {
          if (
            (scrollDirection === "up" && $currentSection.scrollTop() === 0) ||
            (scrollDirection === "down" &&
              $currentSection.scrollTop() + window.windowHeight >=
  ...
  function updateScroll() {
    ...
    $(".slide").each(function (index, element) {
      var $element = $(element),
        ...
        halfWay =
          window.windowHeight / 2 > elementHeight
            ? elementHeight / 2
            : window.windowHeight / 2,
        halfOnScreen =
          positionY < window.windowHeight + scrollTop - halfWay &&
          positionY > scrollTop - elementHeight + halfWay,
        scale =
          ((scrollTop + window.windowHeight - positionY) /
            (window.windowHeight + elementHeight) -
            0.5) *
          2,
        allowToSelect = true;
      ...
      if (
        scale > -1 &&
        scale < 1 &&
        !(window.allowParallaxOnMobile && window.isMobile)
      ) {
        if (
          $element.hasClass("parallax") ||
          $element.find(".parallax-element")
        ) {
          $element.find(".parallax-element").each(function () {
            var $el = $(this),
              velocity = parseInt($el.data("parallax-velocity"))
                ? parseInt($el.data("parallax-velocity"))
                : 50,
  ...
  if ($(".panel.hideOnScroll").length > 0) {
    ...
    if (window.isScroll || window.isSimplifiedMobile) {
      ...
      $(window).on("mousewheel", function (event) {
        ...
        if (
          scrollTop + window.windowHeight + sensitivity >=
            window.documentHeight ||
  ...
  if (window.setPopupHash) {
    ...
    if (
      $.inArray(window.location.hash.split("#")[1], window.setPopupHash) !== -1
    ) {
      setTimeout(function () {
        $(
    ...
    if (gridEl.length) {
      $(gridEl).each(function (index, element) {
        var screenWidth = window.windowWidth,
          ...
          collapseWidth = $(element).data("equal-collapse-width")
            ? parseInt($(element).data("equal-collapse-width"))
            : collapseWidth,
  ...
  $("#contact-form, [data-ajax-form]").each(function (index, element) {
    $(element).ajaxForm(function () {
      var $ajaxForm = $(element),
        ...
        successText = $ajaxFormButton.data("success-text")
          ? $ajaxFormButton.data("success-text")
          : "Done!",
        successClass = $ajaxFormButton.data("success-class")
          ? $ajaxFormButton.data("success-class")
          : "green",
        defaultText = ajaxFormButtonIsInput
          ? $ajaxFormButton.val()
          : $ajaxFormButton.html(),
